...
public interface TourRequestRepo extends JpaRepository<TourRequest, Long> {
    List<TourRequest> findByAgency_IdAndAgentIsNullAndClosedAtIsNull(Long id);
    List<TourRequest> findByAgentIsNullAndClosedAtIsNull();

    List<TourRequest> findAllByUser_Username(String username);
    List<TourRequest> findAllByAgent_Username(String username);
}
